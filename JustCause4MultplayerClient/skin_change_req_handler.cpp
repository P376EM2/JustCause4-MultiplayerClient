...
SkinChangeRequestHandler::SkinChangeRequestHandler()
{
    ...
    std::call_once(_once, [&] {
        ...
        meow_hook::func_call<void>(GetAddress(offsets::ENTITY_PROVIDER_CTOR), m_provider); // CEntityProvider::CEntityProvider
        ...
        pfn_DrawSkinBatches = MH_STATIC_DETOUR(GetAddress(offsets::DRAW_SKIN_BATCHES), DrawSkinBatches);
    });
}
...
void SkinChangeRequestHandler::Request(CSharedString &resource_path, std::function<void(const CRuntimeContainer *)> fn)
{
    ...
    meow_hook::func_call<void>(GetAddress(offsets::ENTITY_PROVIDER_LOAD_RESOURCES), m_provider, &resource_path);
}
...
void SkinChangeRequestHandler::Update()
{
    ...
    meow_hook::func_call<void>(
        GetAddress(offsets::ENTITY_PROVIDER_UPDATE_INTERNAL), m_provider, this,
