        ...
        if (rc.GetHash(HASH_CLASS_HASH) == HASH_CCHARACTER) {
            ...
            if (skin_swapper) {
                if (skin_swapper->m_currentSkin == skin_swapper->m_defaultSkin) {
                    ...
                    meow_hook::func_call<void>(GetAddress(offsets::CHARACTER_MODEL_COPY_PARTS), &skin_swapper->m_oldModelState,
            ...
            meow_hook::func_call<void>(GetAddress(offsets::CHARACTER_MODEL_SET_PROPERTIES), &model_data, &rc);

            // rebuild model parts
            meow_hook::func_call<void>(GetAddress(offsets::CHARACTER_MODEL_REBUILD_MODEL), model_state, &model_data,
    ...
    if (skeleton_hash == HASH_SKEL_WORLD || skeleton_hash == HASH_SKEL_COMBATANT || skeleton_hash == HASH_SKEL_TITAN) {
        for (decltype(model_state->m_modelCount) i = 0; i < model_state->m_modelCount; ++i) {
            ...
            for (decltype(render_block_count) x = 0; x < render_block_count; ++x) {
                ...
                if (vtable == GetAddress(offsets::VTABLE_RENDERBLOCKCHARACTER)
                    || vtable == GetAddress(offsets::VTABLE_RENDERBLOCKCHARACTERSKIN)
                    || vtable == GetAddress(offsets::VTABLE_RENDERBLOCKGENERAL)) {
